// SSGIComputeRayMarching.hlsl
#ifndef SSGI_COMPUTE_RAYMARCHING_HLSL
#define SSGI_COMPUTE_RAYMARCHING_HLSL

// ============================================================================
// Ray march parameters structure
// ============================================================================

struct SSGIRayMarchParams
{
    int maxSteps;
    int maxSmallSteps;
    int maxMediumSteps;
    ...
    half mediumStepSize;
    half thickness;
    half thicknessIncrement;
    int rayCount;
};

SSGIRayMarchParams InitializeRayMarchParams()
{
    SSGIRayMarchParams params;
    params.maxSteps = (int)_MaxSteps;
    params.maxSmallSteps = (int)_MaxSmallSteps;
    params.maxMediumSteps = (int)_MaxMediumSteps;
    params.stepSize = _StepSize;
    params.smallStepSize = _SmallStepSize;
    params.mediumStepSize = _MediumStepSize;
    params.thickness = _Thickness;
    params.thicknessIncrement = _Thickness_Increment;
    params.rayCount = (int)_RayCount;
    return params;
}
...
RayHit RayMarching(Ray ray, float2 screenUV, half dither, half3 viewDirectionWS, SSGIRayMarchParams params)
{
    ...
    for (int i = 1; i <= params.maxSteps; i++)
    {
        ...
        if (i > params.maxSmallSteps && i <= params.maxMediumSteps)
        {
            currStepSize = (startBinarySearch) ? currStepSize : params.maxMediumSteps;
            thickness = (startBinarySearch) ? thickness : MARCHING_THICKNESS_MEDIUM_STEP;
            marchingThickness = params.thickness;
        }
        else if (i > params.maxMediumSteps)
        {
            // [Far] Use a small step size only when objects are close to the camera.
            currStepSize = (startBinarySearch) ? currStepSize : stepSize;
            thickness = (startBinarySearch) ? thickness : params.thickness;
            marchingThickness = params.thickness;
        }
        ...
        float deviceDepth = SAMPLE_TEXTURE2D_X_LOD(_CameraDepthTexture, sampler_PointClamp, rayPositionNDC.xy, 0).r;
        ...
        if (hitSuccessful)
        {
            ...
            if (_BackDepthEnabled = 2.0 && isBackBuffer)
                rayHit.emission = SAMPLE_TEXTURE2D_X_LOD(_CameraBackOpaqueTexture, sampler_PointClamp, rayPositionNDC.xy, 0).rgb;
            else
                rayHit.emission = SAMPLE_TEXTURE2D_X_LOD(_SSGIHistoryCameraColorTexture, sampler_PointClamp, rayPositionNDC.xy, 0).rgb;
...
#endif // SSGI_COMPUTE_RAYMARCHING_HLSL
